...
void IPC::receiveMessage(int task_id)
{
    ...
    if (mailboxes.isNode(task_id))
    {
        ...
        sprintf(buffer, " Source ID: %d\n", mailboxes.searchNode(task_id)->data.top()->data.src_id);
        mcb->mywin->write_window(mcb->mywin->Misc_Table, buffer);

        sprintf(buffer, "Dest ID: %d", mailboxes.searchNode(task_id)->data.top()->data.dst_id);
        mcb->mywin->write_window(mcb->mywin->Misc_Table, buffer);

        sprintf(buffer, " Arrival Time: \n");
        mcb->mywin->write_window(mcb->mywin->Misc_Table, buffer);

        strcpy(buffer, mailboxes.searchNode(task_id)->data.top()->data.arrived_time.c_str());
        mcb->mywin->write_window(mcb->mywin->Misc_Table, buffer);

        sprintf(buffer, " Message Type: \n");
        mcb->mywin->write_window(mcb->mywin->Misc_Table, buffer);
        ...
        strcpy(buffer, mailboxes.searchNode(task_id)->data.top()->data.msg_content.c_str());
        mcb->mywin->write_window(mcb->mywin->Misc_Table, buffer);

        sprintf(buffer, " ----END MESSAGE---\n\n");
        mcb->mywin->write_window(mcb->mywin->Misc_Table, buffer);
    ...
    {
        mcb->mywin->write_window(mcb->mywin->Misc_Table, " Impossible to obtain message, mailbox do not exist!\n");
    }
}
//----------------------------------------
...
{
    sprintf(buffer, " ---- MAILBOX Thread # %d -----\n", task_id);
    mcb->mywin->write_window(mcb->mywin->Misc_Table, buffer);
    ...
    for (int j = 0; j < mailboxes.searchNode(task_id)->data.size(); j++)
    {
        ...
        sprintf(buffer, " Dest ID: %d \n", tmp->data.dst_id);
        mcb->mywin->write_window(mcb->mywin->Misc_Table, buffer);

        mcb->mywin->write_window(mcb->mywin->Misc_Table, " Arrival Time: \n");

        sprintf(buffer, " ");
        strcat(buffer, tmp->data.arrived_time.c_str());
        mcb->mywin->write_window(mcb->mywin->Misc_Table, buffer);

        mcb->mywin->write_window(mcb->mywin->Misc_Table, " Message Type: \n");
        ...
        sprintf(buffer, " ");
        strcat(buffer, tmp->data.msg_content.c_str());
        mcb->mywin->write_window(mcb->mywin->Misc_Table, buffer);

        mcb->mywin->write_window(mcb->mywin->Misc_Table, "\n");

        mcb->mywin->write_window(mcb->mywin->Misc_Table, " ----END MESSAGE---\n\n");
        tmp = tmp->next;
    }
