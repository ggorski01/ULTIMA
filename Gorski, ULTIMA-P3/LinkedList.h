...
//             if not exit with error.
template <class T>
void LinkedList<T>::dumpList(WINDOW *win)
{
    Node<T> *tmp = head;
    if (!empty())
    {
...
//write_window() - To display things at specific
//                 curses window.
template <class T>
void LinkedList<T>::write_window(WINDOW *Win, const char *text)
{
