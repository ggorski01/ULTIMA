...
int Scheduler::create_task()
{
    if (nextAvailableThreadID < NO_OF_THREADS)
    {
        ...
        WINDOW *newTaskWIN = mcb->mywin->create_window(10, 27, 15, 2 + (threadTable.getThreadTableSize() * 28));
...
void Scheduler::yield()
{
    ...
    if (elapsed_time >= currentQuantum) // if quantum has run out
    {
        ...
        currentThread = threadTable.getTCBIter()->threadID;
...
void Scheduler::dump()
{
    ...
    sprintf(buffer, "     TASK #   \t    PROCESS ID \t\t    TASK STATE \n---------------------------------------------------------\n");
    mcb->mywin->write_window(mcb->mywin->Tb_Win, 1, 1, buffer);

    TCB *tmp = threadTable.getTCBHead();
    while (tmp != NULL)
    {
        if (tmp->threadState == READY)
            sprintf(buffer, " \t%d \t|%li \t|READY ", tmp->threadID, tmp->processID);
        else if (tmp->threadState == RUNNING)
            sprintf(buffer, " \t%d \t|%li \t|RUNNING ", tmp->threadID, tmp->processID);
        else if (tmp->threadState == BLOCKED)
            sprintf(buffer, " \t%d \t|%li \t|BLOCKED ", tmp->threadID, tmp->processID);
        else if (tmp->threadState == DEAD)
            sprintf(buffer, " \t%d \t|%li \t|DEAD ", tmp->threadID, tmp->processID);
        ...
        mcb->mywin->write_window(mcb->mywin->Tb_Win, buffer);
        tmp = tmp->next;
    }
    mcb->mywin->write_window(mcb->mywin->Tb_Win, "---------------------------------------------------------\n");

    sprintf(buffer, " Quantum: %li \t| Elapsed Time: %d\t|\n", currentQuantum, elapsed_time);
    mcb->mywin->write_window(mcb->mywin->Tb_Win1, buffer);
}
//----------------------------------------
...
{
    mcb->mywin->write_window(mcb->mywin->Misc_Table, " ...Cleaning Garbage..\n");

    for (int j = 0; j < NO_OF_THREADS; j++)
    {
        if (threadTable.findTCB(j) && threadTable.getTCBState(j) == DEAD)
        {
            threadTable.removeATCB(j);
            mcb->myIPC->removeAMailbox(j);
            break;
        }
    }
}
...
{
    mcb->mywin->write_window(mcb->mywin->Misc_Table, " ...Killing Task..\n");
